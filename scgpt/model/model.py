...
class TransformerModel(nn.Module):
    def __init__(
        ...
        # self.lm_head = nn.Linear(self.d_model, ntoken , bias=False)   #n_embed,vocab_size,bias
        # self.epx_head = nn.Linear(self.d_model, ntoken, bias=False)
        ...
        print('self.input_emb_style',self.input_emb_style)
        ...
        # self.genes_decoder = GenesDecoder(d_model, self.max_seq_len+1, nlayers=nlayers_cls)
        if do_mvc:
    ...
    def generate(
        ...
        if src_key_padding_mask is None:
            # torch.zeros 创建一个新的掩码。这个掩码的形状 (shape) 由 total_embs 的前两个维度决定，
            # 这通常是批次大小和序列长度。
    ...
    def forward(
        ...
        if self.explicit_zero_prob and do_sample:
            ...
            output["mlm_geneid"] = mlm_output["gene_id_preds"]
        ...
        # output["mlm_output_genes"] = self.genes_decoder(cell_emb)
...
class GeneEncoder(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.enc_norm(x)   #(batch:几个句子，seq_len:每个句子几个词，embszie：512)
        return x
...
class ExprDecoder(nn.Module):
    def __init__(
        ...
        self.gene_id_pred = gene_id_pred
        if gene_id_pred:
            self.gene_id_logit = nn.Sequential(
                nn.Linear(d_in, d_model),
                nn.LeakyReLU(),
                nn.Linear(d_model, d_model),
                nn.LeakyReLU(),
    ...
    def forward(self, x: Tensor) -> Dict[str, Tensor]:
        ...
        if not self.explicit_zero_prob  and not self.gene_id_pred:
            return dict(pred=pred_value)
        if self.explicit_zero_prob  and not self.gene_id_pred:
            zero_logits = self.zero_logit(x).squeeze(-1)  # (batch, seq_len)
            zero_probs = torch.sigmoid(zero_logits)
            return dict(pred=pred_value, zero_probs=zero_probs)
        if not self.explicit_zero_prob  and  self.gene_id_pred:
            gene_id_preds = self.gene_id_logit(x).squeeze(-1)  # (batch, seq_len)
            # zero_probs = torch.sigmoid(zero_logits)
            return dict(pred=pred_value, gene_id_preds=gene_id_preds)
...
class GenesDecoder(nn.Module):
    ...
    def __init__(
        ...
        for i in range(nlayers - 1):
            ...
            self._decoder.append(activation())
            self._decoder.append(nn.LayerNorm(d_model))
        self.out_layer = nn.Linear(d_model, n_genes)
